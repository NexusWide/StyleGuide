% The main LaTeX file for the NexusWide StyleGuide git_project_management PDF documentations.


% front_matter:
    % 1. Cover.
    % 2. Title.
    % 3. Copyright.
    % 4. Dedication.
    % 5. Contents.
    % 6. preface.


%
\documentclass[13pt]{scrarticle}
%



% text_formatting_oriented_packages packages.
\usepackage[utf8]{inputenc}
\usepackage[english]{babel}
\usepackage{microtype}
\usepackage[none]{hyphenat}
\usepackage{parskip}
%


% graphical_formatting_oriented packages.
\usepackage{graphicx}
\usepackage{xcolor}
\usepackage{fancyhdr}
\usepackage{tikzsymbols}
%


%
\newcommand{\header}[1]{\textsf{#1}}
\newcommand{\important}[1]{\textit{#1}}
\newcommand{\name}[1]{\color{red} {\textup{#1}}\color{black}}
\newcommand{\response}[1]{\Tribar[2][white][yellow][red]\color{blue} #1}
%


%
\renewcommand{\thesection}{\Roman{section}}
\setlength{\parindent}{0cm}
%


%
\title{NexusWide Project Managemet Guidelines}
\author{Nima Bavar}
\date{\today}
%


\begin{document}
    \raggedright
    \pagestyle{fancy}


    \fancyhf{}
    \rhead{\leftmark}
    \rfoot{\thepage}

        \includegraphics[width=20cm]{/home/nimabavar/Desktop/work/diary/projects/nexus_wide/repositories/style_guide/attachments/nexus_wide_logo.png}
    \pagenumbering{gobble}


    \thispagestyle{empty}
    \pagecolor{white}


    \begin{figure}[t]
        \centering \includegraphics[width=14cm]{/home/nimabavar/Desktop/work/diary/projects/nexus_wide/repositories/style_guide/attachments/nexus_wide_logo.png}
    \end{figure}


    \centering{ \name{ \color{red} \hspace{2.4cm} Research Institute Presents.} \newline
    \thispagestyle{empty}


    \maketitle{}


    \newpage
    \thispagestyle{fancy}


    \section{\header{Copyright Laws}}
    \pagenumbering{Roman}

    \raggedright

    The corresponding documentation literature and all the containment's, are authorized under the \name{Creative Commons Zero v1.0 Universal License} ,
    with the elucidation of the actuality that all branches of \name{modification}, \name{distribution}, \name{patent}, \name{commercial } and \name{private usage } are entirely authorized.
    \newline

    This fragment of dossier is submitted \important{AS IS},
    you are liberate to manufacture any category of modifications you find esstenital for your individuality or organization.
    \newline

    However, this aforementioned fact loose the \name{NexusWide } team from any fork of provisioned liability or warranty regarding the usage scenario of such composition,
    denoting that we are not held accountable for whichever harm caused by the employment/misemployment of this attestation in any form,
    therefore we demand you to cautiously study the above-mentioned license sanctions if the decision for calibrating this file in a
    personal or officially concealed environment was made. \newline

    Accordingly, the \name{NexusWide } research team welcomes all enhancement ideations announced and pushed to the official repository of the imminent writing.
    \newline

    \newpage



\end{document}
