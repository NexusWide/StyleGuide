...
\begin{document}
    ...
    The key words \important{MUST}, \important{MUST NOT}, \important{REQUIRED}, \important{SHALL}, \important{SHALL NOT}, \important{SHOULD}, \important{SHOULD NOT}, \important{RECOMMENDED},  \important{MAY}, and \important{OPTIONAL} in this document are to be interpreted as described in
    \name{The Internet Network Group} Request For Comment \important{number 2119} \newline

    Numerous semantic conventions are globally used throughout this document
    ...
    \subsection{\header{Examples}}
     ...
     \section{\header{Algorithm Designing  Guidelines}}
     \subsection{\header{Preamble}}
     ...
     We courageously demand you to elucidate the \name{Algorithm} conventions of the \name{NexusWide} group
     so that we can guarantee the assessment of our products.
    ...
    \nexusrule{Class attributes MUST visualized using the attribute shape.} \newline

    \nexusrule{Loops starting, finishing and changes in operations MUST be visualized in Square.} \newline

    \nexusrule{Normal logical activities  MUST be visualized in milestone plan item ( not classes, functions, decorators ).} \newline

    \nexusrule{User inputs MUST be visualized in Hexagon shape.} \newline

    \nexusrule{System decisions and exceptions MUST be visualized in Rhombus.} \newline

    \nexusrule{Functions MUST be visualized in Triangles.} \newline

    \nexusrule{Function calls MUST be visualized in call activity shape ( yellow Square ).} \newline

    \nexusrule{Connected attributes MUST be visualized in page-refrences shape.} \newline

    \nexusrule{Class and abstract connections MUST be visualized in the class shape.} \newline

    \nexusrule{Databases MUST be visualized using the database shape.} \newline

    \nexusrule{Connections MUST be visualized in bullet-head lines.} \newline

    \nexusrule{Imports, accesses and inherits MUST be visualized in arrowhead lines.} \newline

    \nexusrule{Partitions MUST be visualized in vertical lines.} \newline

    \nexusrule{Partition names MUST be visualized in the case file item.} \newline

    \nexusrule{Comments MUST be visualized in comment alt shape.} \newline

    \nexusrule{Numerical orderings MUST be visualized in brackets.} \newline

    \nexusrule{Implement indications are visualized in implementation event.} \newline

    \nexusrule{All algorithm files ( excluding the text-based ones ) MUST contain a diagram info object explaining the purpose of the file.} \newline

    \nexusrule{The diagram info object MUST contain an ( end ) object, describing the outcome or the exportation of the algorithm.} \newline

    \nexusrule{All textual subjects MUST be written using the Jetbrains Sans Mono font family.} \newline

    \nexusrule{All algorithms MUST be visualized in Visual Paradigm.} \newline
    ...
    \subsection{Examples}
        ...
        \begin{figure}[h!]
            \fbox{\includegraphics[width=13.5cm, height=6cm]{/home/nimabavar/Desktop/work/diary/projects/nexus_wide/repositories/style_guide/attachments/designing_algorithm_style/example1.png}}
            \caption{Example of \name{NexusWide} \name{Algorithm} design conventions.}
        \end{figure}
